...
export class SearchPlaceComponent {
  ...
  onSearch() {
    this.store.dispatch(new SetSearchQuery(this.searchQuery)).subscribe(() => {
      this.tripadvisorApiService
        .fetchLocations(this.searchQuery)
        .subscribe((locations) =>
          this.store.dispatch(new SetLocations(locations.data))
        );
    });
